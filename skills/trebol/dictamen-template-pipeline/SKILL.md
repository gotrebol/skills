...
- **PDF editable:** los tokens van como **nombre del campo de formulario** (un campo de texto por variable; nunca copiar/pegar el mismo campo). Los bucles **no funcionan en PDF**.
...
2. **Carga el grounding (diccionario de variables).** Lee en silencio — no lo listes al usuario, solo internalízalo:
   ...
   - `grounding/example-actinver-mapping.md` — **ejemplo trabajado de una plantilla de cliente real ya configurada** (Actinver, PDF de formulario). Documenta cómo se mapea un PDF real: el token va en el **valor** del campo (no en el nombre), el mapeo de etiquetas de poderes (incl. `delegate` confirmado y `dominio→assets_management`), las dos variantes de domicilio, los flags del comprobante (`address_service_type_*`), y los slots fijos de apoderados/consejo/UBOs. Léelo cuando configures PDFs o cuando dudes cómo se llena un campo.
